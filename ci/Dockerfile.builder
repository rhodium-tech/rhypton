...
# rhypton_hash
RUN git clone https://github.com/rhyptonpay/rhypton_hash
RUN cd rhypton_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} rhypton
RUN useradd -u ${USER_ID} -g rhypton -s /bin/bash -m -d /rhypton rhypton
...
RUN mkdir /rhypton-src && \
  mkdir -p /cache/ccache && \
  mkdir /cache/depends && \
  mkdir /cache/sdk-sources && \
  chown $USER_ID:$GROUP_ID /rhypton-src && \
