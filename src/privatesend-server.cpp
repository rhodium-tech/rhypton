...
void CPrivateSendServer::ProcessMessage(CNode* pfrom, const std::string& strCommand, CDataStream& vRecv, CConnman& connman)
{
    ...
    if (fLiteMode) return; // ignore all RHYPTON related functionality
...
/*
    ...
    allow endless transaction that would bloat RHYPTON and make it unusable. To
...
{
    if (fLiteMode) return;        // disable all RHYPTON specific functionality
