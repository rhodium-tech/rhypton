...
const char * const BITCOIN_CONF_FILENAME = "rhypton.conf";
const char * const BITCOIN_PID_FILENAME = "rhyptond.pid";
    ...
    if (category != NULL)
    {
        ...
        if (ptrCategory.get() == NULL || *cacheCounter != logAcceptCategoryCacheCounter.load())
        {
            ...
            if (mapMultiArgs.count("-debug")) {
                ...
                // "rhypton" is a composite category enabling all RHYPTON-related debug output
                if(ptrCategory->count(std::string("rhypton"))) {
...
boost::filesystem::path GetDefaultDataDir()
{
    ...
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RHYPTON
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RHYPTON
    // Mac: ~/Library/Application Support/RHYPTON
    // Unix: ~/.rhypton
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RHYPTON";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RHYPTON";
#else
    // Unix
    return pathRet / ".rhypton";
#endif
#endif
}
...
void ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty rhypton.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override rhypton.conf
