...
// which are part of the RHYPTON repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="rhypton-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /rhypton-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "rhyptonpay-rhypton/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from rhyptonpay-rhypton/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'rhyptonpay-rhypton/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from rhyptonpay-rhypton/develop"
            }
          }

          if (hasCache) {
            sh "cd /rhypton-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /rhypton-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /rhypton-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /rhypton-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /rhypton-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /rhypton-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /rhypton-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }
